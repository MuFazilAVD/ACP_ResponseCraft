...
| Overrides | Partial | `context.system_prompt_override` and generation overrides are accepted and forwarded. `model_override` remains a known contract gap because `InvokeRequest.model_override` is disabled and invoke currently hardcodes `gemini-2.5-flash-cto-lab`. |
| LLM | Pass locally | Default model is `gemini-2.5-flash-cto-lab` through the ACP LiteLLM-compatible gateway at `https://d2brdeqy144bwg.cloudfront.net/myllm/v1/` with request user `AgentStudio`; operator completed local live LLM smoke with a real key. Runtime mock LLM drafting is disabled, and LLM failures return invoke errors. |
